...
for(setsnp in c("1240K","1240K.TVs","SG","SG.TVs")){
  ...
  outfile<-paste("F4_NorthRef_toUspallataSouthRef/F4.NorthRef_toUspallataSouthRef.finalSet.Lab_with_Compendium_GEHmodern.",setsnp,".TH50000.",popUSP,".summarized.pdf",sep="")
  f4$Compa<-paste("F4(Mbuti,",f4$Region.North,";\n",popUSP," Uspallata,Ancient South American group)",sep="")
...
write.table(outall[,names(outall)!="Compa"],str_replace(outfile,".pdf",".tsv"),col.names=T,row.names=F,sep="\t",quote=F)

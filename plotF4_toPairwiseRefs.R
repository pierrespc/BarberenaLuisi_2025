...
for( setsnp in c("1240K","1240K.TVs","SG","SG.TVs")){
  ...
  for(focus in c("Uspallata")){
    ...
    outfile<-paste("F4_UspallataOneGroup_toRefs/F4.Uspallata_toPairwiseRefs.finalSet.Lab_with_Compendium_GEHmodern.",setsnp,".TH",th,".pdf",sep="")
    pdf(outfile,width=12,height=12)
    par(mfrow=c(1,1),mar=c(2,2,14,6)+0.1)
